...
$n^{O(\sqrt n)}$. We report the new terms through $n=18$ and describe four independent validations,
...
\item The new values through $n=18$ (Section~\ref{sec:results}), with four independent validations.
\end{enumerate}
...
restricted to seeing at least one block of every boundary orbit (``covers all of $X$''). These are
...
\[
T(\lambda)\ \le\ n^{O(1)}\,P(\lambda)^{O(1)}\,(n+1)^{\,r}.
\]
\end{lemma}
...
($\le\log n$), and evaluates the sub-world $g_1$ at each generating-function monomial (a memo
...
(each step at least halves the largest size). Charging each sub-instance by the same bound multiplies
the total by a factor $n^{O(1)}$, leaving the stated form.
\end{proof}
...
Lemma~\ref{lem:percost}, $T(\lambda)\le n^{O(1)}P(\lambda)^{O(1)}(n+1)^{r}$. By
Lemma~\ref{lem:maxprod}, $P(\lambda)^{O(1)}\le e^{O(\sqrt n)}$ and $r\le\sqrt{2n}$, so
$(n+1)^{r}\le(n+1)^{\sqrt{2n}}=e^{O(\sqrt n\log n)}$; hence $T(\lambda)\le e^{O(\sqrt n\log n)}=
...
$n\le15$ reproduce the previously published values, and $n=16,17,18$ are new (to be submitted to the
OEIS).
...
sub-exponentially ($p(15)=176$, $p(18)=385$). The difficulty is concentrated in the cycle types with
...
$n=18$, so the two independently assembled sequences are mutually consistent at the new terms.
\end{enumerate}
...
\paragraph{Data availability.} The two computed tables ($n=1,\dots,18$) have SHA-256 digests
\begin{center}\footnotesize\ttfamily
\begin{tabular}{ll}
A048192 (connected) & e91c9a6d936007b4814503593fa5522d265eb254414ae484815a5b6e33a625e6\\
A048193 (all) & 13f8305370f6dafdef2756c86ff3847a4ffc0d4f9138217ca3db8b087c111298\\
\end{tabular}
\end{center}
The implementation (the recurrences of Appendix~\ref{app:rec}, the brute-force oracle, the
verification scripts, the b-files, and this paper's source) is publicly available at
\href{https://github.com/msun170/chordal-counting}{\texttt{github.com/msun170/chordal-counting}}.
